...
const lineHeightMin = 21;
const lineHeightMax = 33;
const ratio = 0.80;
...
export default class TabBar extends React.Component {
	...
	componentDidMount() {
		if (Platform.OS === 'ios') {
			this.setTextStyle(this.fonts[0], {fontSize: maxSize, lineHeight: lineHeightMax, });
	...
	setAnimationValue({value}) {
		...
		// console.warn(differ)
		if (differ >= 2) {
			let progress = value * (1 / differ);console.log(progress, '====');
			if (activeTab - this.props.fromIndex > 0) {//往右划
				this.fonts.forEach((font, i) => {
					if (this.props.fromIndex === i) {
						...
						const lineHeight = lineHeightMin * (1 - progress) * ratio;
						this.setTextStyle(font, {fontSize, lineHeight})
					}
					else if (activeTab === i) {
						let fontSize = minSize * (1 + progress) * ratio;
						let lineHeight = lineHeightMin * (1 + progress) * ratio;
						this.setTextStyle(font, {fontSize, lineHeight})
					}
				});
			}
				...
				this.fonts.forEach((font, i) => {
					if (this.props.fromIndex === i) {
						...
						let lineHeight = lineHeightMin * (1 + (1 - progress)) * ratio;
						this.setTextStyle(font, {fontSize, lineHeight})
					}
				});
			}
			...
			if (nextValue > 0) {//往右划
				...
				this.fonts.forEach((font, i) => {
					...
					if (activeTab === i) {
						...
						const lineHeight = lineHeightMin * (1 + (value - activeTab)) * ratio;
				...
				this.fonts.forEach((font, i) => {
					if (activeTab === i) {
						...
						const lineHeight = lineHeightMin * (1 + (activeTab - value)) * ratio;
	...
	setTextStyle = (font, {fontSize, color, lineHeight}) => {
		font && font.setNativeProps({
			style: {
				fontSize: fontSize <= minSize ? minSize : fontSize >= maxSize ? maxSize : fontSize,
				lineHeight: lineHeight <= lineHeightMin ? lineHeightMin : lineHeight >= lineHeightMax ? lineHeightMax: lineHeight,
				color,
			},
		});
	};
		...
		>
			<View style={[styles.tab, this.props.tabStyle,]}>
				...
				<Text
					...
					style={[{color: textColor, fontWeight,}, textStyle, _activeTextStyle]}>
					{name}
				</Text>
			</View>
		</Button>;
